...
export class GameScene {
    ...
    private emitWalk(seq:number){

        var move = false;
        var direction = 1;

        if("KeyW" in this._input.keys){
            move = true;
            direction = 1;
        }else if("KeyA" in this._input.keys){
            move = true;
            direction = 2;
        }else if("KeyS" in this._input.keys){
            move = true;
            direction = 3;
        }else if("KeyD" in this._input.keys){
            move = true;
            direction = 4;
        }

        var playerMove = {direction: direction, seq: seq};

        if(move){
            // sent current input to server for processing 
            this.room.send("walk", playerMove);
            
            // do client side prediction
            this._currentPlayer.moveController.predictionMove(playerMove);
        }
